...
def make_price_trades_event_fig(
    ...
    # --- Row2: Timeline events
    # events_df is already exploded + normalized
    evs = events_df.copy() if events_df is not None else pd.DataFrame()
    if len(evs):
        # 只保留当前 symbol + 允许 symbol 为空(组合级)也显示
        evs_sym = evs[(evs["symbol"].astype(str) == str(symbol)) | (evs["symbol"].astype(str) == "")]
    else:
        evs_sym = evs

    # Add Orders/Fills as timeline points too (optional)
    extra_rows = []
    if orders_df is not None and len(orders_df):
        od = orders_df.copy()
        od["ts"] = pd.to_datetime(od["ts"], errors="coerce")
            ...
            extra_rows.append(
                {
                    ...
                    "etype": "ORDER",
                    "symbol": str(symbol),
                    "payload_json": r.to_dict(),
                    "hover": f"ORDER {symbol}\n{_short_payload(r.to_dict())}",
                }
            )

    if fills_df is not None and len(fills_df):
        fd = fills_df.copy()
        fd["ts"] = pd.to_datetime(fd["ts"], errors="coerce")
    ...
    if extra_rows:
        ...
        evs_sym = pd.concat([evs_sym, extra_df], ignore_index=True).sort_values("ts")

    # plot in fixed order; still show if there are events
    for et in EVENT_ORDER:
        sub = evs_sym[evs_sym["etype"] == et] if len(evs_sym) else evs_sym
        if len(sub):
            fig.add_trace(
                go.Scattergl(
                    x=sub["ts"],
    ...
    fig.update_layout(
        height=900,  # ✅ 更大
        xaxis_rangeslider_visible=False,
        title=f"{symbol} - Price + Trades + Event Timeline",
        hovermode="x unified",  # ✅ 好用的 hover
        legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="left", x=0),
        margin=dict(l=20, r=20, t=60, b=20),
    )
