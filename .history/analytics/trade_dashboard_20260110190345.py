...
def extract_insight_points(
    ...
    def _get_dir(payload):
        if not isinstance(payload, dict):
            return None
        # most common
        for k in ["direction", "dir", "signal", "side", "action", "position"]:
...
with tab_insight:
    if len(bars):
        insight_pts = extract_insight_points(bars_df=bars, events_ex=events_ex, symbol=symbol)
        fig = make_price_insight_fig(bars_df=bars, insight_pts=insight_pts, symbol=symbol)
        st.plotly_chart(fig, width="stretch", key=f"tab_insight_price_{run_id}_{symbol}", config=PLOTLY_CONFIG)

        if insight_pts is not None and len(insight_pts) == 0:
            st.info("No INSIGHT events found for this symbol (or they could not be aligned to bars).")
    else:
