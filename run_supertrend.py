...
from strategies.supertrend_algo import (
    SupertrendConfig,
    SupertrendStrategy
)
...
def main():
    ...
    # 2) Strategy
    algo = SupertrendStrategy(cfg)
    # 3) 三模型
    pc_model = WeightedByHintPC(normalize_gross=True, gross_cap=1.0)
    risk_model = ChainRiskManagementModel([
        StopLossRiskModel(stop_loss_pct=0.5),  # 风控层退出
        PortfolioMaxDrawdownRiskModel(max_drawdown=0.5),             # 组合回撤保护
